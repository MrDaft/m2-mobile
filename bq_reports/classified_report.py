...
try:
    ...
    send_to_ss(classified_mobile, sheet_all, df, f'A{int(get_gs_len(classified_mobile, sheet_all)[0])+2}')
...
try:
    ...
    send_to_ss(classified_mobile, sheet_phone_call, df, f'A{int(get_gs_len(classified_mobile,sheet_phone_call)[0])+2}')
except Exception as e:
    logging.exception(str(e))
